...
public class AutoGetMenu extends WrappedGuiScreen {
        ...
        if (!this.closed) {
            ...
            if (this.mouseOverElement != null && (this.mouseOverElement instanceof ControlElement || this.mouseOverElement instanceof ModuleElement)) {
                ...
                if (element.shouldOpenSubSettings()) {
                    ...
                    if(element.hasSubSettings() && (element instanceof ListContainerElement && ((ListContainerElement)element).openSubSettings())){
                        this.path.add(element);
                        this.scrollbar.setScrollY(0.0D);
                        this.initGui();
                    }
                    if(element instanceof ListContainerElement) {
    ...
    public void selectItem(Item item) {
        if(autoGetModule == null)
            return;
        autoGetModule.selectItem(item);
    }
}
