...
public class TransporterGetMessageHandler implements IMessageHandler {
    ...
    private boolean matchGetManglerMessage(String clean){
        final Pattern pattern = Pattern.compile("Du har ikke nok items i din transporter! \\(0\\)");
    ...
    private boolean matchGetSuccessMessage(String clean){
        final Pattern pattern = Pattern.compile("^Du har taget ([0-9]+) (" + module.getItemRegex() + ") fra din transporter\\. Du har ([0-9]+) \\([0-9]+ stacks\\) " + module.getItemRegex() + " tilbage i den\\.$");
        final Matcher matcher = pattern.matcher(clean);
        if (matcher.find()) {
            ...
            if(mode == MessageModes.NO_MESSAGES) {
                ...
                PlayerAPI.getAPI().displayActionBarMessage(module.getMessage(module.getRawMessage("getSuccess"), item.getDisplayName().toLowerCase(), matcher.group(1), matcher.group(3)));
                return true;
            }else if(mode == MessageModes.CHAT_MESSAGES){
                PlayerAPI.getAPI().displayChatMessage(module.getMessage(module.getRawMessage("getSuccess"), item.getDisplayName().toLowerCase(), matcher.group(1), matcher.group(3)));
                return true;
            }
        }
        return false;
